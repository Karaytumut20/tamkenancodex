...
// Define products array directly in JS to avoid TypeScript transpilation issues during running
const products = [
  {
  ...
  },
  {
    ...
    benefits: [
      ...
      { title: "Anında Müdahale Şansı", description: "Herhangi bir şüpheli harekette telefonunuza gelen canlı video bildirimi sayesinde anında polisi arayabilir veya sesli uyarı verirsiniz." }
    ],
  ...
  },
  {
    ...
    description: "Optik algılama hücresiyle dumanı ve ani sıcaklık artışlarını erkenden tespit eden yangın ihbar dedektörü.",
    ...
    faqs: [
      { question: "Mutfakta yemek pişerken dedektör sürekli çalar mı?", answer: "Mutfak gibi buhar ve yemek dumanı olabilecek alanlarda sadece duman algılayan optik dedektörler yerine, ani ısı artışını izleyen 'Isı Dedektörleri' konumlandırılarak gereksiz alarmların önüne geçecektir." },
  ...
  },
  {
    ...
    benefits: [
      ...
      { title: "Hızlı Maaş ve Hak Ediş Hesaplama", description: "Ay sonunda puantaj hesaplama karmaşasına son verin, saatler süren bordro hazırlığını dakikalara indirin." }
    ],
...
async function run() {
  console.log("Fetching Categories and Brands from Supabase...");
  ...
  if (catErr || brandErr) {
    console.error("Error fetching categories/brands:", catErr || brandErr);
    process.exit(1);
  }
  ...
  for (const prod of products) {
    ...
    const brandId = brandMap[prod.brand];

    if (!categoryId) {
      console.warn(`Category "${prod.category}" not found in DB categories. Skipping product "${prod.name}".`);
      continue;
    }

    if (!brandId) {
      console.warn(`Brand "${prod.brand}" not found in DB brands. Defaulting to PrimeSec.`);
    }
    ...
    if (queryError) {
      console.error(`Error querying product by slug ${prod.slug}:`, queryError);
      continue;
    }
    ...
    const payload = {
      ...
      brand_id: brandId || brandMap["PrimeSec"],
      image_url: prod.image,
      tags: prod.tags,
      usage_areas: prod.usage,
      features: prod.features,
      benefits: prod.benefits,
      installation_steps: prod.specs,
      faqs: prod.faqs,
      meta_title: prod.metaTitle,
      meta_description: prod.metaDescription,
      specs_title: prod.specsTitle,
      specs_description: prod.specsDescription,
      benefits_title: prod.benefitsTitle,
      benefits_description: prod.benefitsDescription,
      show_features: prod.showFeatures !== false,
      show_specs: prod.showSpecs !== false,
      show_benefits: prod.showBenefits !== false,
      is_active: true,
    ...
    if (existingProduct) {
      ...
      if (updateError) {
        console.error(`Error updating product ${prod.slug}:`, updateError);
      } else {
      ...
      if (insertError) {
        console.error(`Error inserting product ${prod.slug}:`, insertError);
      } else {
  ...
  console.log("Products synchronization complete!");
}
