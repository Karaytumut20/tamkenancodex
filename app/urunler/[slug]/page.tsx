        ...
        <Container className="grid gap-10 lg:gap-12 lg:grid-cols-2 xl:items-center">
          ...
          <article className="space-y-6 md:space-y-8 w-full min-w-0">
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-black tracking-tight text-ink leading-tight break-words">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 to-blue-600">ne işe yarar?</span>
            </h1>
            ...
            <div className="space-y-4 md:space-y-6 text-base sm:text-lg leading-7 sm:leading-8 text-ink-muted">
              {copy.overview.map((paragraph, index) => (
                <p key={index} className={`break-words ${index === 0 ? "text-lg sm:text-xl font-medium text-ink/90 leading-relaxed border-l-4 border-cyan-500 pl-4" : ""}`}>
                  {paragraph}
                </p>
              ))}
            </div>
            ...
            <div className="pt-4 md:pt-6">
              ...
              >
                <svg className="h-5 w-5 sm:h-6 sm:w-6 fill-current mr-2 sm:mr-3" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="flex items-center justify-center w-full min-w-0">
            <ProductImageGallery mainImage={product.image} gallery={product.gallery} />
          </div>
        </Container>
      </section>
        ...
        <section className="hero-bg py-16 md:py-24 relative overflow-hidden text-white border-t border-b border-white/5">
          ...
          <Container className="relative z-10">
            <div className="max-w-3xl mx-auto text-center mb-10 md:mb-16">
              <span className="text-[10px] md:text-xs font-extrabold uppercase tracking-widest text-cyan-400">NEDEN BU ÜRÜN?</span>
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      <h3 className="text-lg sm:text-xl font-bold text-ink tracking-tight">{feature.title}</h3>
                      <p className="mt-2 sm:mt-3 text-xs sm:text-sm leading-6 text-ink-muted">
                        {feature.description || "PrimeSec Teknoloji bu avantajı doğru konumlandırma, kaliteli montaj ve üstün servis süreciyle sürekli kılar."}
                      </p>
                    </div>
          ...
          <Container className="grid gap-10 lg:gap-12 lg:grid-cols-2 lg:items-center">
            ...
            <article className="space-y-4 md:space-y-6 w-full min-w-0">
              ...
              <p className="text-sm sm:text-base md:text-lg leading-relaxed text-ink-muted break-words">
                {product.specsDescription || `${product.name} modeli, en zorlu kullanım koşullarında bile kesintisiz performans göstermek üzere tasarlanmıştır. İşte öne çıkan teknik konfigürasyonlar:`}
              </p>
              ...
              <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 pt-2 md:pt-4">
                {(product.specs && product.specs.length > 0 ? product.specs : [
                  { title: "Kategori", description: product.category, active: true },
                  { title: "Marka", description: product.brand, active: true },
                  { title: "Ürün Kodu", description: product.code || "PS-GEN-001", active: true },
                  { title: "Garanti", description: "2 Yıl Kurumsal", active: true }
                ]).filter((item: any) => item.active !== false).map((item, index) => {
                  let SpecIcon = Cpu;
                  const lowerTitle = item.title.toLowerCase();
                  if (lowerTitle.includes("bağlantı") || lowerTitle.includes("wi-fi") || lowerTitle.includes("network") || lowerTitle.includes("kod")) {
                    SpecIcon = Zap;
                  } else if (lowerTitle.includes("garanti") || lowerTitle.includes("güvenlik") || lowerTitle.includes("koruma")) {
                    SpecIcon = ShieldCheck;
                  } else if (lowerTitle.includes("çözünürlük") || lowerTitle.includes("kamera") || lowerTitle.includes("görüntü") || lowerTitle.includes("kategori")) {
                    SpecIcon = Eye;
                  } else if (lowerTitle.includes("kullanım") || lowerTitle.includes("alan")) {
                    SpecIcon = Layers;
                  } else if (lowerTitle.includes("marka") || lowerTitle.includes("üretici")) {
                    SpecIcon = Sparkles;
                  }
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2 sm:gap-3 mb-1 sm:mb-2">
                        ...
                        <div className="text-[10px] sm:text-xs font-bold text-slate-400 tracking-wider uppercase break-words">{item.title}</div>
                      </div>
                      <div className="text-sm sm:text-base font-extrabold text-ink leading-snug break-words pl-1">{item.description}</div>
                    </div>
                  );
                })}
              </div>
        ...
        <section className="bg-white py-12 md:py-24 border-b border-slate-100/80 overflow-hidden relative">
          ...
          <Container className="grid gap-10 lg:gap-12 lg:grid-cols-2 lg:items-center">
            ...
            <article className="space-y-4 md:space-y-6 w-full min-w-0 order-2 lg:order-1">
              ...
              <p className="text-sm sm:text-base md:text-lg leading-relaxed text-ink-muted break-words">
                {product.benefitsDescription || "Güvenlik sisteminizle her an bağlantıda kalın. PrimeSec tarafından kurulan tüm sistemler, tek bir mobil uygulama üzerinden anlık bildirimler, uzaktan izleme ve tam kontrol sunarak akıllı ekosisteminize kusursuz uyum sağlar."}
              </p>
            </article>
      ...
      <section className="bg-surface py-16 md:py-24 border-b border-border">
        <Container>
          <div className="max-w-3xl mx-auto text-center mb-12 md:mb-16 px-2">
            <span className="text-[10px] md:text-xs font-extrabold uppercase tracking-widest text-primary-600">SÜREÇ AKIŞI</span>
          ...
          <div className="relative grid gap-8 sm:gap-6 grid-cols-1 sm:grid-cols-2 md:grid-cols-4">
            ...
            {process.map((step, index) => (
              <div key={step} className="relative z-10 flex flex-col items-center text-center group">
                ...
                <h3 className="mt-4 sm:mt-6 text-lg sm:text-xl font-extrabold text-ink">{step}</h3>
      ...
      <section className="bg-white py-12 md:py-20 border-b border-border">
        <Container>
          <div className="mb-8 md:mb-10 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div>
              <span className="text-[10px] md:text-xs font-extrabold uppercase tracking-widest text-primary-600">TAMAMLAYICI ÜRÜNLER</span>
              <h2 className="mt-1 md:mt-2 text-2xl sm:text-3xl md:text-4xl font-extrabold text-ink">İlgili Ürünler</h2>
            </div>
            <Link
              href="/urunler"
          ...
          <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            {(related.length ? related : allProducts.filter((p) => p.slug !== product.slug).slice(0, 4)).map((item) => (
            ...
            <div className="relative z-10 grid gap-6 md:gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
              <div>
                <div className="inline-flex items-center gap-1.5 sm:gap-2 px-3 py-1 rounded-full bg-white text-primary-600 text-[10px] sm:text-xs font-semibold uppercase tracking-wider mb-4 sm:mb-6">
                  <ClipboardCheck className="h-3 w-3 sm:h-4 sm:w-4" /> Ücretsiz Danışmanlık
                </div>
...
function buildProductCopy(product: Product) {
  const defaultOverview = [
    ...
    `Bir güvenlik ürününden beklenen şey yalnızca çalışması değildir; doğru yerde, doğru senaryoda ve doğru entegrasyonla çalışmasıdırl. ${product.name} kurulurken alanın giriş noktaları, kullanım yoğunluğu, internet veya network altyapısı, kayıt ihtiyacı ve mobil bildirim beklentisi birlikte değerlendirilir.`,
