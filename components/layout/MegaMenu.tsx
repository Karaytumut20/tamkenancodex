          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {menu.items.map((item) => (
              <Link
                ...
                onClick={onNavigate}
                className="group grid grid-cols-[104px_1fr_auto] items-center gap-4 rounded-xl px-4 hover:bg-slate-50/70 hover:translate-x-1 transition-all duration-300 py-2.5"
              >
                <span className="relative h-24 w-24 overflow-hidden rounded-xl">
                  <Image src={item.image} alt={item.title} fill sizes="96px" className="object-contain p-0.5 transition-transform duration-300 group-hover:scale-105" />
                </span>
