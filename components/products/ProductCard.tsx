...
export function ProductCard({ product }: { product: Product }) {
  return (
    <article className="group flex h-full min-w-0 flex-col rounded-xl border border-border bg-white p-4 transition-all duration-300 hover:-translate-y-1 hover:border-cyan-500 hover:shadow-md sm:rounded-2xl sm:p-5">
      <Link href={`/urunler/${product.slug}`} className="product-card-image relative mb-4 block overflow-hidden rounded-lg border border-border/70 bg-surface sm:rounded-xl">
        <Image src={product.image} alt={`${product.name} ürün görseli`} fill sizes="(min-width: 1024px) 33vw, (min-width: 520px) 50vw, 100vw" className="object-contain p-4 sm:p-5" unoptimized />
      </Link>
      ...
      <h2 className="min-w-0 text-base font-extrabold leading-6 text-ink sm:text-lg sm:leading-normal">
      ...
      <div className="mt-auto grid gap-2 pt-3 sm:pt-4">
        <ButtonLink href={`/urunler/${product.slug}`} variant="primary" size="sm" className="w-full min-w-0 rounded-lg px-2 text-sm sm:rounded-xl">Detayları İncele</ButtonLink>
        <ButtonLink href={productWhatsappUrl(product)} variant="whatsapp" size="sm" className="product-card-whatsapp w-full min-w-0 rounded-lg px-2 text-sm sm:rounded-xl">
