...
export function SystemBuilder({ initialGroups }: { initialGroups: SystemBuilderGroup[] }) {
  ...
  const [selectedProducts, setSelectedProducts] = useState<SystemBuilderItem[]>([]);
  ...
  function toggleProduct(product: SystemBuilderItem) {
  ...
  async function submit(values: LeadForm) {
    await fetch("/api/lead", {
      ...
      body: JSON.stringify({
        ...
        floor,
        reason,
        selectedProducts: selectedProducts.map(p => p.title),
                  ...
                  {step === 1 ? (
                    <StepBlock title="Alarm sisteminle nereyi korumak istersin?">
                      <div className="grid gap-4 md:grid-cols-2">
                        <SelectionCard title="Evimi" description="Daire, villa, yazlık ve yaşam alanları" selected={area === "Evimi"} onClick={() => { setArea("Evimi"); setFloor(""); setReason(""); setSelectedProducts([]); }} icon={<Home className="h-8 w-8" />} image="/images/local-security.svg" />
                        <SelectionCard title="İş Yerimi" description="Mağaza, ofis, depo ve ticari alanlar" selected={area === "İş Yerimi"} onClick={() => { setArea("İş Yerimi"); setFloor(""); setReason(""); setSelectedProducts([]); }} icon={<Briefcase className="h-8 w-8" />} image="/images/network.svg" />
                      </div>
                    </StepBlock>
                  ) : null}
                  {step === 2 ? (
                  ...
                  {step === 4 ? (
                    <StepBlock title="Paketine hangi ürünleri eklemek istersin?">
                      <div className="mb-6 rounded-2xl border border-primary-500/20 bg-white p-5 text-sm leading-7 text-ink-muted">
                        {area === "İş Yerimi"
                          ? "Uzun süre kapalı kalan iş yerleri güvenlik riski oluşturabilir. İhtiyaç duyduğunuz kamera ve alarm bileşenlerini seçerek iş yerinizi güvence altına alabilirsiniz."
                          : "Girişe açık evler hırsızlık ve su baskınına karşı daha hassas olabilir. İhtiyaç duyduğunuz sensör ve kameraları aşağıdan seçebilirsiniz."}
                      </div>
                      ...
                      {initialGroups.length > 0 ? (
                        ...
                        <div className="p-8 text-center border-2 border-dashed border-border rounded-xl text-ink-muted font-bold">
                          Kendi Sistemini Tasarla ürünleri admin panelden ayarlanmalıdır. Henüz ürün bulunmuyor.
                        </div>
                      )}
...
function ProductSection({ title, badge, products, selected, onToggle }: { title: string; badge?: string; products: SystemBuilderItem[]; selected: SystemBuilderItem[]; onToggle: (value: SystemBuilderItem) => void }) {
  return (
    <div className="mb-8">
      {title && (
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4">
        {products.map((item) => {
          ...
          return (
            <button key={item.id} onClick={() => onToggle(item)} className={cn("relative flex min-h-[148px] flex-col items-center justify-between gap-3 rounded-xl border-2 bg-white p-4   ", active ? "border-primary-500 bg-white" : "border-border")}>
...
function SummaryCard({ area, floor, reason, products }: { area: string; floor: string; reason: string; products: SystemBuilderItem[] }) {
  return (
      ...
      <div className="mt-6 rounded-2xl border border-border bg-surface p-4">
        ...
        <div className="mt-3 space-y-2.5">
          {products.slice(0, 5).map((product) => (
            <div key={product.id} className="flex items-center gap-2 text-sm font-semibold text-ink-muted line-clamp-1">
              <Check className="h-4 w-4 shrink-0 text-primary-600" /> <span className="truncate">{product.title}</span>
            </div>
          ))}
...
function FinalOffer({ submitted, area, floor, reason, selectedProducts, onBack, form, onSubmit }: { submitted: boolean; area: string; floor: string; reason: string; selectedProducts: SystemBuilderItem[]; onBack: () => void; form: ReturnType<typeof useForm<LeadForm>>; onSubmit: (values: LeadForm) => void }) {
  return (
        ...
        <div className="mt-10 grid gap-7 xl:grid-cols-12">
          ...
          <div className="rounded-[24px] border border-border bg-white p-6 md:p-8 xl:col-span-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h2 className="text-2xl font-extrabold text-ink">Seçtiğiniz {area === "İş Yerimi" ? "İş Yeri" : "Ev"} Ürünleri</h2>
              <span className="rounded-full border border-primary-200 bg-white px-4 py-2 text-sm font-extrabold text-primary-600">Size Özel Paket</span>
            </div>
